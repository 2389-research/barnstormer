// ABOUTME: LLM provider status detection for the specd UI.
...
impl ProviderStatus {
    ...
    /// - SPECD_DEFAULT_PROVIDER / SPECD_DEFAULT_MODEL
    ///
...
mod tests {
    ...
    fn detect_finds_default_provider() {
        ...
        assert_eq!(
            ...
            "default provider should be 'anthropic' when SPECD_DEFAULT_PROVIDER is not set"
        );

        // Now set a custom default provider
        // SAFETY: holding ENV_MUTEX, no concurrent env var access
        unsafe {
            std::env::set_var("SPECD_DEFAULT_PROVIDER", "openai");
        }
    ...
    fn detect_default_model_override() {
        ...
        unsafe {
            clear_provider_env();
            std::env::set_var("SPECD_DEFAULT_MODEL", "claude-opus-4-20250918");
        }
