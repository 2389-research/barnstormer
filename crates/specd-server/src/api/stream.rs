...
fn event_type_name(payload: &specd_core::EventPayload) -> &'static str {
    match payload {
        specd_core::EventPayload::SpecCreated { .. } => "spec_created",
        specd_core::EventPayload::SpecCoreUpdated { .. } => "spec_core_updated",
        specd_core::EventPayload::CardCreated { .. } => "card_created",
        specd_core::EventPayload::CardUpdated { .. } => "card_updated",
        specd_core::EventPayload::CardMoved { .. } => "card_moved",
        specd_core::EventPayload::CardDeleted { .. } => "card_deleted",
        specd_core::EventPayload::TranscriptAppended { .. } => "transcript_appended",
        specd_core::EventPayload::QuestionAsked { .. } => "question_asked",
        specd_core::EventPayload::QuestionAnswered { .. } => "question_answered",
        specd_core::EventPayload::AgentStepStarted { .. } => "agent_step_started",
        specd_core::EventPayload::AgentStepFinished { .. } => "agent_step_finished",
        specd_core::EventPayload::UndoApplied { .. } => "undo_applied",
        specd_core::EventPayload::SnapshotWritten { .. } => "snapshot_written",
    }
}

/// Convert a broadcast receiver into an SSE-compatible stream.
fn event_stream_from_receiver(
    rx: tokio::sync::broadcast::Receiver<specd_core::Event>,
