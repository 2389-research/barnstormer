// ABOUTME: High-level storage manager for the specd daemon's filesystem layout.
// ABOUTME: Handles directory creation, spec discovery, recovery orchestration, and export writing.

use std::fs;
use std::path::{Path, PathBuf};

use specd_core::export::{export_dot, export_markdown, export_yaml};
...
/// Manages the specd home directory layout and provides high-level operations
...
mod tests {
    ...
    fn storage_manager_creates_directories() {
        ...
        let home = dir.path().join("specd_home");
